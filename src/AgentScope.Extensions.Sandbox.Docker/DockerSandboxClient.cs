...
/// <summary>
/// Docker 沙箱客户端（薄工厂）。对标 Java DockerSandboxClient。
/// </summary>
public sealed class DockerSandboxClient : ISandboxClient
{
    ...
    private readonly DockerSandboxClientOptions _options;

    public DockerSandboxClient(DockerSandboxClientOptions? options = null)
    {
        _options = options ?? new DockerSandboxClientOptions();
    }

    public Task<ISandbox> CreateAsync(WorkspaceSpec spec, CancellationToken ct = default)
    {
        var sandbox = new DockerSandbox(_options.Image, _options.ContainerName);
        return Task.FromResult<ISandbox>(sandbox);
    }

    public Task<ISandbox> ResumeAsync(SandboxState state, CancellationToken ct = default)
    {
        var s = DockerSandboxState.FromSandboxState(state);
        var sandbox = new DockerSandbox(s.Image ?? _options.Image, s.ContainerName);
        return Task.FromResult<ISandbox>(sandbox);
    }
    ...
    public SandboxState DeserializeState(string json)
        => JsonSerializer.Deserialize<SandboxState>(json, JsonOpts)
           ?? throw new InvalidOperationException("Failed to deserialize Docker sandbox state.");
