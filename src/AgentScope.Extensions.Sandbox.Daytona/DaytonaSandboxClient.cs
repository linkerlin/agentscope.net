...
/// <summary>
/// Daytona 沙箱客户端（薄工厂）。对标 Java DaytonaSandboxClient。
/// </summary>
public sealed class DaytonaSandboxClient : ISandboxClient
{
    ...
    private readonly DaytonaSandboxClientOptions _options;
    private readonly HttpClient _http;

    public DaytonaSandboxClient(DaytonaSandboxClientOptions? options = null, HttpClient? http = null)
    {
        _options = options ?? new DaytonaSandboxClientOptions();
        _http = http ?? new HttpClient();
    }

    public Task<ISandbox> CreateAsync(WorkspaceSpec spec, CancellationToken ct = default)
    {
        var sandbox = new DaytonaSandbox(_http, _options.ControlPlaneBaseUrl);
        return Task.FromResult<ISandbox>(sandbox);
    }

    public Task<ISandbox> ResumeAsync(SandboxState state, CancellationToken ct = default)
    {
        var s = DaytonaSandboxState.FromSandboxState(state);
        var sandbox = new DaytonaSandbox(_http, _options.ControlPlaneBaseUrl);
        return Task.FromResult<ISandbox>(sandbox);
    }
    ...
    public SandboxState DeserializeState(string json)
        => JsonSerializer.Deserialize<SandboxState>(json, JsonOpts)
           ?? throw new InvalidOperationException("Failed to deserialize Daytona sandbox state.");
