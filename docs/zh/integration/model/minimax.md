...
Provider 的 base URL 默认使用 `https://api.minimaxi.com/v1`。`OpenAIClient` 会追加默认 chat completions endpoint，因此最终请求 URL 是 `https://api.minimaxi.com/v1/chat/completions`，与 MiniMax OpenAI 兼容 API 保持一致。发送请求前会去掉 `minimax:` 前缀，并使用 `AgentScope.Extensions.Model.OpenAI.Compat.MiniMax` 下的 MiniMax formatter。
