...
```csharp
var market = new NacosSkillRepository(aiService, "namespace");
HarnessAgent.Builder()
        .SkillRepository(market)
        .Build();
```

适合需要在线下发、变更订阅的场景。`market` 是 `IDisposable`，应用退出时关掉以释放订阅。

### MySQL

```csharp
var registry = MysqlSkillRepository.Builder(dataSource)
...
.EnableSkillPromotionGate(
    ...
    new CompositeFilter(new List<ISkillVisibilityFilter> {          // 怎么暴露
...
```csharp
var entries = agent.QueryAudit(LocalDate.Now(), e => true);
