...
HarnessAgent.Builder()
    ...
    .Subagent(SubagentDeclaration.Builder()
        .Name("reviewer")
        .Description("代码审查专家")
        .Workspace(Path.GetRelativePath("./defs/reviewer"))
        .WorkspaceMode(WorkspaceMode.ISOLATED)
        .Model("qwen3-max")
        .Steps(8)
        .Tools(new List<string> { "read_file", "grep_files" })
        .Build())
    .Subagent(SubagentDeclaration.Builder()
...
await foreach (var evt in chat.SendStream(SendOptions.UserId("user-1"), "派一个研究员调查 AI 趋势"))
{
    if (evt is SubagentExposedEvent se)
    {
        // se.SubagentId → 用来直接和子 agent 对话
        // se.AgentId    → 子 agent 类型（如 "researcher"）
        // se.Label      → 可选的人类可读名称
    }
}

// 直接向暴露的子 agent 发消息
chat.SendToSubagent(subagentId, "重点关注 LLM agent").GetAwaiter().GetResult();
```
...
```csharp
var decl = SubagentDeclaration.Builder()
    .Name("researcher")
...
var chat = agent.Channel(ChatUiChannel.Create());  // 恢复能力自动接好
```
...
- **流式转发**：父 agent `stream()` 时，同步子 agent 的中间事件会实时流回父的流（带来源标记），见下文 [子 Agent 流式](#子-agent-流式)。
...
.Subagent(SubagentDeclaration.Builder()
    ...
    .RemoteContextAttributes(new Dictionary<string, string> { ["region"] = "cn" })
    .Build())
```
...
| 字段 | 默认 | 说明 |
|------|------|------|
| `remoteStreaming` | `true`（未设置时） | 父代理使用 `StreamEventsAsync()` 时，把远程任务的 SSE 事件转发进父流，并带 `source` 标记与 `metadata.taskId` / `metadata.parentSessionId`（与 harness `TaskRecord` / 父 session 一致） |
...
- **父代理流式 + `remoteAskPolicy=PROPAGATE`**：向父的 `StreamEventsAsync()` 转发带非空 `source` 标记的 `RequireUserConfirmEvent`。通过 Agent Protocol [`POST /tasks/{id}/resume`](../../integration/protocol/agent-protocol.md) 恢复，请求体为 `decisions[{toolCallId, approved}]`。
...
### 使用 `StreamEventsAsync()`（推荐）

```csharp
await foreach (var evt in parent.StreamEventsAsync(new UserMessage(message), ctx))
{
    var src = evt.GetSource();
    var prefix = (src != null) ? "[" + src + "] " : "";

    if (evt.Type == AgentEventType.TEXT_BLOCK_DELTA)
    {
        Console.Write(prefix + ((TextBlockDeltaEvent) evt).Delta);
    }
    else if (evt.Type == AgentEventType.TOOL_CALL_START)
    {
        Console.WriteLine(prefix + "[tool] " + ((ToolCallStartEvent) evt).ToolCallName);
    }
    else if (evt.Type == AgentEventType.AGENT_START)
    {
        if (src != null) Console.WriteLine("── 子 agent 启动: " + src);
    }
    else if (evt.Type == AgentEventType.AGENT_END)
    {
        if (src != null) Console.WriteLine("── 子 agent 结束: " + src);
    }
}
```
...
```csharp
// 只看父事件
events.Where(e => e.GetSource() == null).Subscribe(…);

// 只看子事件
events.Where(e => e.GetSource() != null).Subscribe(…);

// 只看特定子 agent 的事件
events.Where(e => e.GetSource() != null && e.GetSource().Contains("researcher")).Subscribe(…);
```
...
{
    var ctx = RuntimeContext.Builder().SessionId(sessionId).Build();
    await foreach (var evt in agent.StreamEventsAsync(new UserMessage(message), ctx))
    {
        ...
        {
            payload["source"] = evt.GetSource();
        }
        if (evt is TextBlockDeltaEvent delta)
        {
            payload["delta"] = delta.Delta;
        }
        else if (evt is ToolCallStartEvent start)
        {
            payload["toolName"] = start.ToolCallName;
        }
...
| 场景 | 是否实时流转发？ |
|------|-----------------|
| `StreamEventsAsync()` + 同步本地子 agent（`timeout_seconds > 0`） | ✔ |
| `CallAsync()` 模式（非流式） | ✗（子结果以 `tool_result` 字符串返回） |
| `timeout_seconds = 0` 后台任务 | ✗（终态会通过反向通知给父 agent 下一轮） |
| 远程子 agent（Agent Protocol）+ 父 `StreamEventsAsync()` + `remoteStreaming=true`（默认） | ✔ |
| 远程子 agent + 父 `CallAsync()` 或 `remoteStreaming=false` | ✗ |

### 错误处理

子 agent 内部出错时，框架会把错误捕获并写成一条 `TOOL_RESULT` 给父，**不会**把 `OnError` 传播到父流——父流不会被子 agent 的失败打断。如果父流本身出错（比如模型调用失败），按标准语义处理。
...
- [消息与事件](../building-blocks/message-and-event.md) — `AgentEvent` 体系以及已弃用的 `Event` / `EventType` / `StreamOptions`
- [V1 迁移指南 B.4](../change-log.md) — `stream()` → `StreamEventsAsync()` 弃用时间线
