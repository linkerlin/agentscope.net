...
每次 `call()` 进入 reasoning 阶段时，`WorkspaceContextMiddleware` 会按下表把工作区文件拼成一段文本，**追加到** builder 上配置的 `sysPrompt` 之后形成最终 system 消息：
...
需要写文件时，**通过 `HarnessAgent.GetWorkspaceManager()` 而不是 `System.IO.File`**——后者在沙箱或共享存储模式下会写到错的地方（写到了宿主磁盘而不是沙箱里 / KV 里）。例外：builder 装配时的初始化脚本（例如 `initWorkspaceIfAbsent` 这种生成 `AGENTS.md` 种子），那时还没有运行时上下文，用 `System.IO.File` 是 OK 的——它本来就是要落本地模板。
