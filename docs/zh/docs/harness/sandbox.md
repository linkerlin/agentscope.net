...
agent.CallAsync(msg, RuntimeContext.Builder()
    .UserId("alice")
    .SessionId("conv-1")
...
.Filesystem(new DockerFilesystemSpec()
    ...
    .ExecutionGuard(RedisSandboxExecutionGuard.Builder(jedis)
        .LeaseTtl(TimeSpan.FromMinutes(30)).Build()))
```
...
```csharp
var mySandbox = dockerClient.Create(workspaceSpec, snapshotSpec, options);
mySandbox.Start();

var callCtx = SandboxContext.Builder()
    .Client(dockerClient)
    .ExternalSandbox(mySandbox)       // 框架在 call 结束时只 Stop()，不 Shutdown()
    .Build();

agent.CallAsync(msgs, RuntimeContext.Builder()
...
```csharp
var savedState = dockerClient.DeserializeState(savedStateJson);
var callCtx = SandboxContext.Builder()
    .Client(dockerClient)
...
Kubernetes 后端完全基于 agent-sandbox：沙箱 pod 由 agent-sandbox 控制器管理，镜像、资源、存储都声明在集群侧的 `SandboxTemplate` / `SandboxWarmPool` 里，C# 侧只负责领取（`SandboxClaim`）和连接。这带来一个和其他后端不同的点——**工作区数据的持久化主要靠 PVC，而不是 Harness 快照**，两层机制各管一事：
