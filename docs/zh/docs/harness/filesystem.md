...
var store = RedisDistributedStore.FromJedis(jedis);
...
```csharp
var store = RedisDistributedStore.FromJedis(
...
Kubernetes 后端完全基于 [agent-sandbox](https://github.com/kubernetes-sigs/agent-sandbox)：沙箱 pod 由集群里的 agent-sandbox 控制器管理，镜像、资源、PVC 都声明在集群侧的 `SandboxTemplate` / `SandboxWarmPool` 里（不在 C# 侧配置），C# 侧通过 `SandboxClaim` 从预热池领取实例。使用前需要先安装 agent-sandbox 控制器并创建好模板和预热池。
...
```csharp
var codingAgent = HarnessAgent.Builder()
    .Name("coder")
    .Model(model)
...
var rc = RuntimeContext.Builder()
    .UserId("alice")
    .SessionId("dev-session-1")
    .Build();
agent.CallAsync(Msg.User("npm install && npm test"), rc).GetAwaiter().GetResult();

// alice 第二次调用：恢复快照，node_modules 还在，无需重新安装
agent.CallAsync(Msg.User("npm run build"), rc).GetAwaiter().GetResult();
```
...
```csharp
var devHelper = HarnessAgent.Builder()
    .Name("dev-helper")
    .Model(model)
    .Workspace(Paths.Get(".agentscope/workspace"))
    .Filesystem(new LocalFilesystemSpec()
