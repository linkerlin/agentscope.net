...
{
    Console.WriteLine("[{0}] {1}", t.GetState(), t.GetSubject());
...
如果需要通过事件流实时感知任务变更，可以在 `StreamEventsAsync()` 中监听 `todo_write` 工具调用：

```csharp
agent.StreamEventsAsync(message)
