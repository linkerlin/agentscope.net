...
AgentScope .NET 2.0 从"构建一个智能体"的工具箱，迈向**面向生产环境运行智能体**的完整平台。本次升级围绕三大主题展开，每一部分都对应一个具体要解决的问题。

:::{note}
AgentScope .NET 2.0 版本尽量保持了对 1.x 版本的兼容，确保大部分用户的平滑升级；但同时 2.0 也带来了 API 层面的不兼容变更，并在核心抽象、API 和架构上均有大幅改进。完整迁移指南详见 [V1 迁移指南](./change-log.md)。
:::
