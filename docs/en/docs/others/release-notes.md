---
title: "Release Notes"
description: "Per-version change records for AgentScope .NET"
---

This page tracks per-version changes for AgentScope .NET 2.0. For the overall migration guide from 1.x, see the [V1 Migration Guide](../change-log.md).
...
AgentScope .NET 2.0.1 is the first maintenance release after 2.0.0 GA. It expands the model-provider ecosystem, hardens Harness subagent / HITL / permission behavior, and fixes a set of production-critical issues.
...
AgentScope .NET 2.0.0 is now Generally Available. This is the first production-ready release of the 2.0 line, marking a milestone in AgentScope .NET's evolution from "transparent development" to "system engineering."

**Quick links:** [Quickstart](../quickstart.md) | [V1 Migration Guide](../change-log.md) | [Going to Production](going-to-production.md)

### 2.0 Core Design Overview

AgentScope .NET 2.0 is a systematic upgrade centered on one goal: **enabling agents to reliably complete tasks**. Here is an overview of its core design:
...
- **`AgentResultEvent`** — new event type emitted when an agent finishes processing, immediately before `AgentEndEvent`, carrying the final `Msg` result. Consumers of `streamEvents()` can obtain the result directly from the event stream without separately subscribing to the `Task<Msg>` return value
