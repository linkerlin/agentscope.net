...
If you're using a local `AgentStateStore` (the default `JsonFileAgentStateStore`) with sandbox mode, the framework logs a warning at build time reminding you that sandbox state won't survive process restarts and can't be shared across instances.
...
agent.Call(msgs, RuntimeContext.Builder()
    ...
    .Put(typeof(SandboxContext), callCtx)
    .Build()).GetAwaiter().GetResult();
