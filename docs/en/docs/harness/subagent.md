...
HarnessAgent.Builder()
    ...
    .Subagent(SubagentDeclaration.Builder()
        ...
        .Workspace(Path.GetFullPath("./defs/reviewer"))
...
await foreach (var event in chat.SendStream(SendOptions.UserId("user-1"), "Spawn a researcher to investigate AI trends"))
{
    ...
    {
        // se.SubagentId → use this to talk directly to the subagent
        // se.AgentId    → subagent type (e.g. "researcher")
        // se.Label      → optional human-readable name
    }
}

// Send a message directly to the exposed subagent
chat.SendToSubagent(subagentId, "Focus on LLM agents specifically").GetAwaiter().GetResult();
```
...
- **Streaming forwarding**: during the parent's `Stream()`, intermediate events from synchronous subagents are forwarded back into the parent's event stream live (with source tags); see [Subagent streaming](#subagent-streaming) below.
...
> New code should use `StreamEvents()` (returns `IAsyncEnumerable<AgentEvent>`). The legacy `Stream()` family is deprecated since 2.0.0 — see [Message & Event](../building-blocks/message-and-event.md) and [V1 Migration Guide B.4](../change-log.md).
...
```csharp
await foreach (var event in parent.StreamEvents(new UserMessage(message), ctx))
{
    string? src = event.Source;
    string prefix = (src != null) ? "[" + src + "] " : "";

    if (event.Type == AgentEventType.TEXT_BLOCK_DELTA)
    {
        Console.Write(prefix + ((TextBlockDeltaEvent) event).Delta);
    }
    else if (event.Type == AgentEventType.TOOL_CALL_START)
    {
        Console.WriteLine(prefix + "[tool] " + ((ToolCallStartEvent) event).ToolCallName);
    }
    else if (event.Type == AgentEventType.AGENT_START)
    {
        if (src != null) Console.WriteLine("── child started: " + src);
    }
    else if (event.Type == AgentEventType.AGENT_END)
    {
        if (src != null) Console.WriteLine("── child finished: " + src);
    }
}
```
...
```csharp
// parent events only
events.Where(e => e.Source == null).Subscribe(…);

// child events only
events.Where(e => e.Source != null).Subscribe(…);

// events from a specific child
events.Where(e => e.Source != null && e.Source.Contains("researcher")).Subscribe(…);
```
                                           ...
                                           [FromQuery] string sessionId)
{
    ...
    await foreach (var event in agent.StreamEvents(new UserMessage(message), ctx))
    {
        ...
        else if (event is ToolCallStartEvent start)
        {
...
When a child throws internally, the framework captures it and writes a `TOOL_RESULT` back to the parent. It **does not** propagate errors into the parent stream — child failures don't break the parent. If the parent stream itself errors, use standard error handling semantics.
...
- [Message & Event](../building-blocks/message-and-event.md) — `AgentEvent` hierarchy (recommended) and the deprecated event types
