...
HarnessAgent agent = HarnessAgent.Builder()
    .Name("MyAgent")
    .Model(model)
    .Workspace(Path.GetFullPath(".agentscope/workspace"))   // omit → see resolution order below
...
|----------|--------|-------|
| 1 | `Workspace(string)` / `Workspace(Path)` | Explicit builder value, overrides everything |
...
| 4 | Default | `Directory.GetCurrentDirectory()/.agentscope/workspace` |
...
Before every reasoning step, `WorkspaceContextMiddleware` assembles the following sections and **appends them to** the `sysPrompt` you set on the builder to form the final system message:
...
1. `ProjectGlobalSkillsDir(string)` — project global, e.g. `~/.agentscope/skills/`
2. `SkillRepository(...)` — marketplace stores (Git / Nacos / MySQL / embedded)
...
When you need to write files, **go through `HarnessAgent.GetWorkspaceManager()`, not raw `File.WriteAllText`** — the latter writes to the wrong place under sandbox or shared-store modes (it lands on the host disk rather than inside the sandbox / in the KV). Exception: builder-time bootstrap scripts (e.g. an `InitWorkspaceIfAbsent` that seeds `AGENTS.md`) — there is no runtime context yet, and direct file I/O is correct because the intent is to write the local template.
