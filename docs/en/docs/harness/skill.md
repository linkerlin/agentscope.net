...
```xml
<PackageReference Include="AgentScope.Extensions.Skill.GitRepository" Version="$(AgentScopeVersion)" />
```
...
```xml
<PackageReference Include="AgentScope.Extensions.NacosSkill" Version="$(AgentScopeVersion)" />
```
...
Best for online distribution + change subscription. `market` is `IDisposable`; dispose it on shutdown to release subscriptions.
...
Ship skills inside your assembly:
...
```csharp
.SkillRepository(new EmbeddedSkillRepository("skills"))
```

Works with both standard libraries and ASP.NET Core applications.
...
|----------|--------|------------------|
| 1 (lowest) | Project-global dir | `ProjectGlobalSkillsDir(string)`, e.g. `~/.agentscope/skills/` |
...
.EnableSkillPromotionGate(
    ...
    new CompositeFilter(new List<ISkillFilter>                     // how to expose
    {
...
```csharp
List<SkillAuditLog.Entry> entries = agent.QueryAudit(DateTime.Now.Date, e => true);

agent.RunCuratorOnce()                                       // run a curation now (bypasses throttle)
     .Subscribe(report => Console.WriteLine(report));
...
| Marketplace — Git / MySQL / Nacos / embedded (Layer 2) | preloaded into memory by the backend |
...
| Marketplace skills (Layer 1 project-global, Layer 2 Git / MySQL / Nacos / embedded) | start as in-memory bytes | `/workspace/.skills-cache/<source>/<name>` |
...
| Method | Effect |
|--------|--------|
| `WorkspaceProjectionRoots(List<string>)` | customize which roots are projected (default includes `skills`, `.skills-cache`) |
