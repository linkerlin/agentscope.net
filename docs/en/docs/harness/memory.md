...
1. **End of every `Call()`** — the default `MemoryFlushMiddleware` behaviour. Can be retuned to `NEVER` or `THROTTLED(TimeSpan)` via `FlushTrigger`.
...
Both flush and offload are **asynchronous**: they are launched in a fire-and-forget fashion after the response stream has ended, so they never block the current `Call()` return. The caller receives the full response first; the flush LLM call and JSONL offload run in the background afterward.
...
HarnessAgent.Builder()
    ...
    .Memory(MemoryConfig.Builder()
        .FlushTrigger(MemoryConfig.FlushTrigger.Throttled(TimeSpan.FromMinutes(10)))
        .Build())
    .Build();
```
...
```csharp
.Memory(MemoryConfig.Builder()
    .ConsolidationMinGap(TimeSpan.FromHours(2))   // background merge at most every 2h
...
| `FlushTrigger` | `FlushTrigger.Always()` | `ALWAYS` / `NEVER` / `THROTTLED(TimeSpan)` |
