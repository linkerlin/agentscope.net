...
All model providers have moved to independent model extension packages, while `AgentScope.Core` keeps only the shared model contracts. See [Model](../docs/building-blocks/model.md) for the full creation paths, ASP.NET Core setup, formatters, credentials, and advanced registry behavior.

| Provider | NuGet package | `ModelRegistry` id | Standard environment variable | Docs |
|----------|---------------|--------------------|-------------------------------|------|
...
```{note}
`AgentScope.Extensions.Model.E2eTests` is a repository test module, not a user-facing model integration dependency.
```
...
Isolated code execution stores. Docker is built-in; the rest are standalone extension packages.
...
```{note}
For ASP.NET Core users, most of the above extensions ship a matching `AgentScope.AspNetCore.*` for one-line integration that removes the manual wiring.
```
