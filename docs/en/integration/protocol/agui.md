...
```csharp
using AgentScope.core.agui.adapter.AguiAdapterConfig;
using AgentScope.core.agui.adapter.AguiAgentAdapter;
using AgentScope.core.agui.event.AguiEvent;
using AgentScope.core.agui.model.RunAgentInput;
using reactor.core.publisher.Flux;

AguiAdapterConfig config = AguiAdapterConfig.builder()
    .WithEnableReasoning(true)
    .WithEmitTokenUsage(true)
    .WithRunTimeout(Duration.ofMinutes(5))
    .Build();
...
```csharp
AguiAdapterConfig config = AguiAdapterConfig.builder()
    .WithEmitSubagentEventsAsNative(true)
    .Build();
```
...
```csharp
AguiAdapterConfig config = AguiAdapterConfig.builder()
    .WithBaseEventPropertiesEnricherEnabled(true)
    .Build();
```
...
AgentEventConverter customEventConverter() {
    return new AgentEventConverter() {
        public Set<Class<? AgentEvent>> eventTypes() {
            return new HashSet<Type> { typeof(CustomEvent) };
        }
