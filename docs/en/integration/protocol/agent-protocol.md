...
```csharp
@Bean
AgentFactory agentFactory(Dictionary<string, HarnessAgent> agentsByName) {
    return request => {
...
```csharp
Dictionary<string, Object> attributes = ctx.Get(AgentProtocolConstants.RUNTIME_CONTEXT_ATTRIBUTES_KEY);
...
RuntimeContextCustomizer tenantContext(TenantService tenants) {
    return (request, builder) => {
        ...
        if (tenant != null) {
            builder.Put(typeof(TenantInfo), tenants.Load(tenant));
        }
    };
}
```
...
SubagentDeclaration.builder()
        .WithName("researcher")
        .WithDescription("Remote researcher")
        .WithUrl("http://remote:8080")
...
RuntimeContext.builder()
        ...
        .Put(AgentSpawnTool.CTX_REMOTE_CONTEXT_ATTRIBUTES, new Dictionary<string, string> { "tenant", "acme"))
        .Build();
```
